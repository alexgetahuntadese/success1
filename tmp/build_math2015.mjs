...
const htmlDir = path.join(process.cwd(), "tmp", "math2015html");
const updatesScript = fs.readFileSync(path.join(process.cwd(), "tmp", "fix_math2015_answers.mjs"), "utf8");
const updatesMatch = updatesScript.match(/const updates = (\{[\s\S]*?\n\});/);
const updates = updatesMatch
  ? Function(`"use strict"; return (${updatesMatch[1]});`)()
  : {};
...
questions.forEach((item) => {
  const id = `m2015-math-${item.number}`;
  const update = updates[id];
  const correctAnswer = update ? update[0] : -1;
  const explanation = update ? update[1] : "No explanation was provided in the source.";
  ...
  lines.push("    ],");
  lines.push(`    correctAnswer: ${correctAnswer},`);
  lines.push(`    explanation: ${JSON.stringify(explanation)},`);
  lines.push("  },");
});
...
fs.writeFileSync(path.join(process.cwd(), "src", "data", "matric2015MathQuestions.ts"), `${lines.join("\n")}\n`);
console.log(`Wrote ${questions.length} questions with ${Object.keys(updates).length} updates applied.`);
