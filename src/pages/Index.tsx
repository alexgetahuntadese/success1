    ...
    <div className="app-shell pt-14 px-4 pb-4 md:p-8 md:pt-14">
      ...
      <div className="max-w-5xl mx-auto relative z-10 flex flex-col items-center w-full py-12 md:py-20">
        <div className="text-center mb-16 opacity-0 animate-fade-in" style={{ animationFillMode: "forwards" }}>
          ...
          <p className="max-w-lg mx-auto text-lg md:text-xl app-muted">
            {t("index.subtitle")}
          </p>
        </div>
          ...
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="app-glass relative rounded-2xl p-8 transition-all duration-500 group-hover:bg-white/[0.12] group-hover:border-white/[0.22] group-hover:scale-[1.02]">
                  ...
                  <p className="mb-6 text-sm text-white/72">{feature.description}</p>
                  <div className="flex items-center text-sm text-white/65 transition-colors duration-300 group-hover:text-white/90">
        ...
        <div className="w-full max-w-5xl mb-16 opacity-0 animate-fade-in" style={{ animationDelay: "0.65s", animationFillMode: "forwards" }}>
          <div className="app-glass rounded-[2rem] p-6 md:p-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
              <div>
                <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/[0.14] bg-white/[0.08] px-3 py-1 text-xs uppercase tracking-[0.2em] text-white/80">
                ...
                <p className="max-w-2xl text-sm md:text-base text-white/72">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {notesByGrade.map((grade) => (
                <button
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-white/[0.14] bg-white/[0.08] p-5 text-left transition-all duration-300 hover:bg-white/[0.12] hover:border-white/[0.22]"
                >
                  ...
                  <div className="mb-4 text-xs text-white/70">{grade.subjects.length} subjects</div>
                  <div className="flex items-center text-xs text-white/65 transition-colors duration-300 group-hover:text-white/90">
              ...
              {stats.map((stat, i) => {
                ...
                return (
                  <div key={i} className="text-center">
                    <Icon className="mx-auto mb-2 h-5 w-5 text-white/65" />
                ...
                <div className="space-y-3">
                  {recentAttempts.map((attempt) => (
                    <div
                      ...
                      className="flex items-center justify-between rounded-xl border border-white/[0.14] bg-white/[0.08] p-3 transition-colors hover:bg-white/[0.12]"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-white truncate">{attempt.subject}</p>
                        <p className="truncate text-xs text-white/70">{attempt.chapter} • Grade {attempt.grade}</p>
                      </div>
                      <div className="flex items-center gap-2 ml-3">
                        <Badge className="text-xs border-white/20 text-white/85 capitalize" variant="outline">
                          {attempt.difficulty}
                        </Badge>
                      ...
                      <span className="ml-3 whitespace-nowrap text-xs text-white/65">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {testimonials.map((item, i) => (
                    <div key={i} className="rounded-xl border border-white/[0.14] bg-white/[0.08] p-4">
                      <p className="mb-3 text-sm italic text-white/78">"{item.quote}"</p>
